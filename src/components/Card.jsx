...
const Card = () => {
    ...
    const handleRent = () => {
        toast.warning('You have no enough balance', { position: 'top-left' })
    }
    return (
                ...
                {data.map((item, index) => {
                    return (
                        <div key={index} className="flex flex-col rounded-lg p-1 items- bg-slate-200 abt card shadow-2xl">
                            ...
                            <button onClick={handleRent} className="bg-green-300 hover:bg-green-600 text-black font-bold py-2 px-4 rounded">
                                Rent
                            </button>
                        </div>
                    )
